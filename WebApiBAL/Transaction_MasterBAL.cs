...
/// <summary>
/// Summary description for Transaction_MasterBAL
/// </summary>
namespace WebApiBAL 
{
    public class Transaction_MasterBAL  : IBaseBAL<Transaction_MasterDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Transaction_MasterDAL objTransaction_MasterDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Transaction_MasterBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objTransaction_MasterDAL=new Transaction_MasterDAL();
        }
        #endregion       

        #region Public Methods
        public async Task<ResultPT> InsertAsync(Transaction_MasterDTO objTransaction_MasterDTO)
        { 
			try
            {
				objResultPT = new ResultPT();
				using (Transaction_MasterDAL objTransaction_MasterDAL = new Transaction_MasterDAL())
                {
                    objResultPT.ReturnObject = await objTransaction_MasterDAL.InsertAsync(objTransaction_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Transaction_Master");
        ...
        public async Task<ResultPT> UpdateAsync(Transaction_MasterDTO objTransaction_MasterDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Transaction_MasterDAL objTransaction_MasterDAL = new Transaction_MasterDAL())
                {
                    objResultPT.ReturnObject = await objTransaction_MasterDAL.UpdateAsync(objTransaction_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Transaction_Master");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Transaction_MasterDAL objTransaction_MasterDAL = new Transaction_MasterDAL())
                {
                    objResultPT.ReturnObject = await objTransaction_MasterDAL.DeleteAsync(ID);
...
////			using (Transaction_MasterDAL objTransaction_MasterDAL = new Transaction_MasterDAL())
////            {
////                 objResultPT.ReturnObject = objTransaction_MasterDAL.DeleteAll();
////                objResultPT.TransactionStatus = Enums.ResultStatus.Success;
...
////    }
        ...
        public async Task<ResultPT> GetByIDAsync(long ID)
        {
			try
            {
				...
				using (Transaction_MasterDAL objTransaction_MasterDAL = new Transaction_MasterDAL())
                {                    
					DataSet dset = await objTransaction_MasterDAL.GetByIDAsync(ID);
					if (dset.Tables.Count > 0)
					{
                        DataTable dt = dset.Tables[0];
						objResultPT.ReturnObject = CommonFunctions.FillProperties<Transaction_MasterDTO>(dt.Rows[0]);
						objResultPT.ResultMsg = Messages.Success;
                        objResultPT.TransactionStatus = Enums.ResultStatus.Success;
					}
					else 
        ...
        public async Task<ResultPT> GetAllAsync( )
        {
			try
            {
				...
				using (Transaction_MasterDAL objTransaction_MasterDAL = new Transaction_MasterDAL())
                {
                    objResultPT.ReturnObject = await objTransaction_MasterDAL.GetAllAsync();                   
                }
				return objResultPT;
			}
            catch (Exception Ex)
  ...
  //          using (Transaction_MasterDAL objTransaction_MasterDAL = new Transaction_MasterDAL())
  //          {
  //              return await objTransaction_MasterDAL.FilterAsync(param);
  //          }
  //      }
