...
using WebApiHelper;
/// <summary>
/// Summary description for Customer_MasterBAL
/// </summary>
namespace WebApiBAL
{
    public class Customer_MasterBAL  : IBaseBAL<Customer_MasterDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Customer_MasterDAL objCustomer_MasterDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Customer_MasterBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objCustomer_MasterDAL=new Customer_MasterDAL();
        }
        #endregion       

        #region Public Methods
        public async Task<ResultPT> InsertAsync(Customer_MasterDTO objCustomer_MasterDTO)
        { 
			try
            {
				objResultPT = new ResultPT();
				using (Customer_MasterDAL objCustomer_MasterDAL = new Customer_MasterDAL())
                {
                    objResultPT.ReturnObject = await objCustomer_MasterDAL.InsertAsync(objCustomer_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Customer_Master");
        ...
        public async Task<ResultPT> UpdateAsync(Customer_MasterDTO objCustomer_MasterDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Customer_MasterDAL objCustomer_MasterDAL = new Customer_MasterDAL())
                {
                    objResultPT.ReturnObject = await objCustomer_MasterDAL.UpdateAsync(objCustomer_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Customer_Master");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Customer_MasterDAL objCustomer_MasterDAL = new Customer_MasterDAL())
                {
                    objResultPT.ReturnObject = await objCustomer_MasterDAL.DeleteAsync(ID);
...
////			using (Customer_MasterDAL objCustomer_MasterDAL = new Customer_MasterDAL())
////            {
////                 objResultPT.ReturnObject = objCustomer_MasterDAL.DeleteAll();
////                objResultPT.TransactionStatus = Enums.ResultStatus.Success;
...
////    }
        ...
        public async Task<ResultPT> GetByIDAsync(long ID)
        {
			try
            {
				...
				using (Customer_MasterDAL objCustomer_MasterDAL = new Customer_MasterDAL())
                {                    
					DataSet dset = await objCustomer_MasterDAL.GetByIDAsync(ID);
					if (dset.Tables.Count > 0)
					{
                        DataTable dt = dset.Tables[0];
						objResultPT.ReturnObject = CommonFunctions.FillProperties<Customer_MasterDTO>(dt.Rows[0]);
						objResultPT.ResultMsg = Messages.Success;
                        objResultPT.TransactionStatus = Enums.ResultStatus.Success;
					}
					else 
        ...
        public async Task<ResultPT> GetAllAsync( )
        {
			try
            {
				...
				using (Customer_MasterDAL objCustomer_MasterDAL = new Customer_MasterDAL())
                {
                    objResultPT.ReturnObject = await objCustomer_MasterDAL.GetAllAsync();                   
                }
				return objResultPT;
			}
            catch (Exception Ex)
  ...
  //          using (Customer_MasterDAL objCustomer_MasterDAL = new Customer_MasterDAL())
  //          {
  //              return await objCustomer_MasterDAL.FilterAsync(param);
  //          }
  //      }
