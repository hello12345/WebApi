...
/// <summary>
/// Summary description for Service_Sales_DetailsBAL
/// </summary>
namespace WebApiBAL
{
    public class Service_Sales_DetailsBAL  : IBaseBAL<Service_Sales_DetailsDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Service_Sales_DetailsDAL objService_Sales_DetailsDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Service_Sales_DetailsBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objService_Sales_DetailsDAL=new Service_Sales_DetailsDAL();
        }
        #endregion       

        #region Public Methods
        public async Task<ResultPT> InsertAsync(Service_Sales_DetailsDTO objService_Sales_DetailsDTO)
        { 
			try
            {
				objResultPT = new ResultPT();
				using (Service_Sales_DetailsDAL objService_Sales_DetailsDAL = new Service_Sales_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objService_Sales_DetailsDAL.InsertAsync(objService_Sales_DetailsDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Service_Sales_Details");
        ...
        public async Task<ResultPT> UpdateAsync(Service_Sales_DetailsDTO objService_Sales_DetailsDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Service_Sales_DetailsDAL objService_Sales_DetailsDAL = new Service_Sales_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objService_Sales_DetailsDAL.UpdateAsync(objService_Sales_DetailsDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Service_Sales_Details");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Service_Sales_DetailsDAL objService_Sales_DetailsDAL = new Service_Sales_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objService_Sales_DetailsDAL.DeleteAsync(ID);
...
////			using (Service_Sales_DetailsDAL objService_Sales_DetailsDAL = new Service_Sales_DetailsDAL())
////            {
////                 objResultPT.ReturnObject = objService_Sales_DetailsDAL.DeleteAll();
////                objResultPT.TransactionStatus = Enums.ResultStatus.Success;
...
////    }
        ...
        public async Task<ResultPT> GetByIDAsync(long ID)
        {
			try
            {
				...
				using (Service_Sales_DetailsDAL objService_Sales_DetailsDAL = new Service_Sales_DetailsDAL())
                {                    
					DataSet dset = await objService_Sales_DetailsDAL.GetByIDAsync(ID);
					if (dset.Tables.Count > 0 && dset.Tables[0].Rows.Count>0)
					{
                        DataTable dt = dset.Tables[0];
						objResultPT.ReturnObject = CommonFunctions.FillProperties<Service_Sales_DetailsDTO>(dt.Rows[0]);
						objResultPT.ResultMsg = Messages.Success;
                        objResultPT.TransactionStatus = Enums.ResultStatus.Success;
					}
					else 
        ...
        public async Task<ResultPT> GetAllAsync( )
        {
			try
            {
				...
				using (Service_Sales_DetailsDAL objService_Sales_DetailsDAL = new Service_Sales_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objService_Sales_DetailsDAL.GetAllAsync();                   
                }
				return objResultPT;
			}
            catch (Exception Ex)
  ...
  //          using (Service_Sales_DetailsDAL objService_Sales_DetailsDAL = new Service_Sales_DetailsDAL())
  //          {
  //              return await objService_Sales_DetailsDAL.FilterAsync(param);
  //          }
  //      }
