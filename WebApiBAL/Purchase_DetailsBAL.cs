...
using WebApiHelper;
/// <summary>
/// Summary description for Purchase_DetailsBAL
/// </summary>
namespace WebApiBAL 
{
    public class Purchase_DetailsBAL  : IBaseBAL<Purchase_DetailsDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Purchase_DetailsDAL objPurchase_DetailsDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Purchase_DetailsBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objPurchase_DetailsDAL=new Purchase_DetailsDAL();
        }
        #endregion       
        ...
        public async Task<ResultPT> UpdateAsync(Purchase_DetailsDTO objPurchase_DetailsDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Purchase_DetailsDAL objPurchase_DetailsDAL = new Purchase_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objPurchase_DetailsDAL.UpdateAsync(objPurchase_DetailsDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Purchase_Details");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Purchase_DetailsDAL objPurchase_DetailsDAL = new Purchase_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objPurchase_DetailsDAL.DeleteAsync(ID);
...
////    }
  ...
  //          using (Purchase_DetailsDAL objPurchase_DetailsDAL = new Purchase_DetailsDAL())
  //          {
  //              return await objPurchase_DetailsDAL.FilterAsync(param);
  //          }
  //      }
