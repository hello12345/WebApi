...
using WebApiCommon;
/// <summary>
/// Summary description for Cat_MasterBAL
/// </summary>
namespace WebApiBAL 
{
    public class Cat_MasterBAL  : IBaseBAL<Cat_MasterDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Cat_MasterDAL objCat_MasterDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Cat_MasterBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objCat_MasterDAL=new Cat_MasterDAL();
        }
        #endregion       
        ...
        public async Task<ResultPT> UpdateAsync(Cat_MasterDTO objCat_MasterDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Cat_MasterDAL objCat_MasterDAL = new Cat_MasterDAL())
                {
                    objResultPT.ReturnObject = await objCat_MasterDAL.UpdateAsync(objCat_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Cat_Master");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Cat_MasterDAL objCat_MasterDAL = new Cat_MasterDAL())
                {
                    objResultPT.ReturnObject = await objCat_MasterDAL.DeleteAsync(ID);
...
////    }
  ...
  //          using (Cat_MasterDAL objCat_MasterDAL = new Cat_MasterDAL())
  //          {
  //              return await objCat_MasterDAL.FilterAsync(param);
  //          }
  //      }
