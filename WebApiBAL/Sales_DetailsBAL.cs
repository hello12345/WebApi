...
/// <summary>
/// Summary description for Sales_DetailsBAL
/// </summary>
namespace WebApiBAL
{
    public class Sales_DetailsBAL  : IBaseBAL<Sales_DetailsDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Sales_DetailsDAL objSales_DetailsDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Sales_DetailsBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objSales_DetailsDAL=new Sales_DetailsDAL();
        }
        #endregion       

        #region Public Methods
        public async Task<ResultPT> InsertAsync(Sales_DetailsDTO objSales_DetailsDTO)
        { 
			try
            {
				objResultPT = new ResultPT();
				using (Sales_DetailsDAL objSales_DetailsDAL = new Sales_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objSales_DetailsDAL.InsertAsync(objSales_DetailsDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Sales_Details");
        ...
        public async Task<ResultPT> UpdateAsync(Sales_DetailsDTO objSales_DetailsDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Sales_DetailsDAL objSales_DetailsDAL = new Sales_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objSales_DetailsDAL.UpdateAsync(objSales_DetailsDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Sales_Details");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Sales_DetailsDAL objSales_DetailsDAL = new Sales_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objSales_DetailsDAL.DeleteAsync(ID);
...
////			using (Sales_DetailsDAL objSales_DetailsDAL = new Sales_DetailsDAL())
////            {
////                 objResultPT.ReturnObject = objSales_DetailsDAL.DeleteAll();
////                objResultPT.TransactionStatus = Enums.ResultStatus.Success;
...
////    }
        ...
        public async Task<ResultPT> GetByIDAsync(long ID)
        {
			try
            {
				...
				using (Sales_DetailsDAL objSales_DetailsDAL = new Sales_DetailsDAL())
                {                    
					DataSet dset = await objSales_DetailsDAL.GetByIDAsync(ID);
					if (dset.Tables.Count > 0 && dset.Tables[0].Rows.Count>0)
					{
                        DataTable dt = dset.Tables[0];
						objResultPT.ReturnObject = CommonFunctions.FillProperties<Sales_DetailsDTO>(dt.Rows[0]);
						objResultPT.ResultMsg = Messages.Success;
                        objResultPT.TransactionStatus = Enums.ResultStatus.Success;
					}
					else 
        ...
        public async Task<ResultPT> GetAllAsync( )
        {
			try
            {
				...
				using (Sales_DetailsDAL objSales_DetailsDAL = new Sales_DetailsDAL())
                {
                    objResultPT.ReturnObject = await objSales_DetailsDAL.GetAllAsync();                   
                }
				return objResultPT;
			}
            catch (Exception Ex)
  ...
  //          using (Sales_DetailsDAL objSales_DetailsDAL = new Sales_DetailsDAL())
  //          {
  //              return await objSales_DetailsDAL.FilterAsync(param);
  //          }
  //      }
