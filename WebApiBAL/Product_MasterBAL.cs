...
using WebApiHelper;
/// <summary>
/// Summary description for Product_MasterBAL
/// </summary>
namespace WebApiBAL 
{
    public class Product_MasterBAL  : IBaseBAL<Product_MasterDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Product_MasterDAL objProduct_MasterDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Product_MasterBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objProduct_MasterDAL=new Product_MasterDAL();
        }
        #endregion       

        #region Public Methods
        public async Task<ResultPT> InsertAsync(Product_MasterDTO objProduct_MasterDTO)
        { 
			try
            {
				objResultPT = new ResultPT();
				using (Product_MasterDAL objProduct_MasterDAL = new Product_MasterDAL())
                {
                    objResultPT.ReturnObject = await objProduct_MasterDAL.InsertAsync(objProduct_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Product_Master");
        ...
        public async Task<ResultPT> UpdateAsync(Product_MasterDTO objProduct_MasterDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Product_MasterDAL objProduct_MasterDAL = new Product_MasterDAL())
                {
                    objResultPT.ReturnObject = await objProduct_MasterDAL.UpdateAsync(objProduct_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Product_Master");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Product_MasterDAL objProduct_MasterDAL = new Product_MasterDAL())
                {
                    objResultPT.ReturnObject = await objProduct_MasterDAL.DeleteAsync(ID);
...
////			using (Product_MasterDAL objProduct_MasterDAL = new Product_MasterDAL())
////            {
////                 objResultPT.ReturnObject = objProduct_MasterDAL.DeleteAll();
////                objResultPT.TransactionStatus = Enums.ResultStatus.Success;
...
////    }
        ...
        public async Task<ResultPT> GetByIDAsync(long ID)
        {
			try
            {
				...
				using (Product_MasterDAL objProduct_MasterDAL = new Product_MasterDAL())
                {                    
					DataSet dset = await objProduct_MasterDAL.GetByIDAsync(ID);
					if (dset.Tables.Count > 0)
					{
                        DataTable dt = dset.Tables[0];
						objResultPT.ReturnObject = CommonFunctions.FillProperties<Product_MasterDTO>(dt.Rows[0]);
						objResultPT.ResultMsg = Messages.Success;
                        objResultPT.TransactionStatus = Enums.ResultStatus.Success;
					}
					else 
        ...
        public async Task<ResultPT> GetAllAsync( )
        {
			try
            {
				...
				using (Product_MasterDAL objProduct_MasterDAL = new Product_MasterDAL())
                {
                    objResultPT.ReturnObject = await objProduct_MasterDAL.GetAllAsync();                   
                }
				return objResultPT;
			}
            catch (Exception Ex)
  ...
  //          using (Product_MasterDAL objProduct_MasterDAL = new Product_MasterDAL())
  //          {
  //              return await objProduct_MasterDAL.FilterAsync(param);
  //          }
  //      }
