...
/// <summary>
/// Summary description for Sales_MasterBAL
/// </summary>
namespace WebApiBAL
{
    public class Sales_MasterBAL  : IBaseBAL<Sales_MasterDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Sales_MasterDAL objSales_MasterDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Sales_MasterBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objSales_MasterDAL=new Sales_MasterDAL();
        }
        #endregion       
        ...
        public async Task<ResultPT> UpdateAsync(Sales_MasterDTO objSales_MasterDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Sales_MasterDAL objSales_MasterDAL = new Sales_MasterDAL())
                {
                    objResultPT.ReturnObject = await objSales_MasterDAL.UpdateAsync(objSales_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Sales_Master");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Sales_MasterDAL objSales_MasterDAL = new Sales_MasterDAL())
                {
                    objResultPT.ReturnObject = await objSales_MasterDAL.DeleteAsync(ID);
...
////    }
  ...
  //          using (Sales_MasterDAL objSales_MasterDAL = new Sales_MasterDAL())
  //          {
  //              return await objSales_MasterDAL.FilterAsync(param);
  //          }
  //      }
