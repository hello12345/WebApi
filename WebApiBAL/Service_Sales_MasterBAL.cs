...
/// <summary>
/// Summary description for Service_Sales_MasterBAL
/// </summary>
namespace WebApiBAL
{
    public class Service_Sales_MasterBAL  : IBaseBAL<Service_Sales_MasterDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Service_Sales_MasterDAL objService_Sales_MasterDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Service_Sales_MasterBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objService_Sales_MasterDAL=new Service_Sales_MasterDAL();
        }
        #endregion       

        #region Public Methods
        public async Task<ResultPT> InsertAsync(Service_Sales_MasterDTO objService_Sales_MasterDTO)
        { 
			try
            {
				objResultPT = new ResultPT();
				using (Service_Sales_MasterDAL objService_Sales_MasterDAL = new Service_Sales_MasterDAL())
                {
                    objResultPT.ReturnObject = await objService_Sales_MasterDAL.InsertAsync(objService_Sales_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Service_Sales_Master");
        ...
        public async Task<ResultPT> UpdateAsync(Service_Sales_MasterDTO objService_Sales_MasterDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Service_Sales_MasterDAL objService_Sales_MasterDAL = new Service_Sales_MasterDAL())
                {
                    objResultPT.ReturnObject = await objService_Sales_MasterDAL.UpdateAsync(objService_Sales_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Service_Sales_Master");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Service_Sales_MasterDAL objService_Sales_MasterDAL = new Service_Sales_MasterDAL())
                {
                    objResultPT.ReturnObject = await objService_Sales_MasterDAL.DeleteAsync(ID);
...
////			using (Service_Sales_MasterDAL objService_Sales_MasterDAL = new Service_Sales_MasterDAL())
////            {
////                 objResultPT.ReturnObject = objService_Sales_MasterDAL.DeleteAll();
////                objResultPT.TransactionStatus = Enums.ResultStatus.Success;
...
////    }
        ...
        public async Task<ResultPT> GetByIDAsync(long ID)
        {
			try
            {
				...
				using (Service_Sales_MasterDAL objService_Sales_MasterDAL = new Service_Sales_MasterDAL())
                {                    
					DataSet dset = await objService_Sales_MasterDAL.GetByIDAsync(ID);
					if (dset.Tables.Count > 0)
					{
                        DataTable dt = dset.Tables[0];
						objResultPT.ReturnObject = CommonFunctions.FillProperties<Service_Sales_MasterDTO>(dt.Rows[0]);
						objResultPT.ResultMsg = Messages.Success;
                        objResultPT.TransactionStatus = Enums.ResultStatus.Success;
					}
					else 
        ...
        public async Task<ResultPT> GetAllAsync( )
        {
			try
            {
				...
				using (Service_Sales_MasterDAL objService_Sales_MasterDAL = new Service_Sales_MasterDAL())
                {
                    objResultPT.ReturnObject = await objService_Sales_MasterDAL.GetAllAsync();                   
                }
				return objResultPT;
			}
            catch (Exception Ex)
  ...
  //          using (Service_Sales_MasterDAL objService_Sales_MasterDAL = new Service_Sales_MasterDAL())
  //          {
  //              return await objService_Sales_MasterDAL.FilterAsync(param);
  //          }
  //      }
