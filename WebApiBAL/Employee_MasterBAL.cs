...
using WebApiHelper;
/// <summary>
/// Summary description for Employee_MasterBAL
/// </summary>
namespace WebApiBAL 
{
    public class Employee_MasterBAL  : IBaseBAL<Employee_MasterDTO>,IDisposable
    {
    	#region Private Variables
		//readonly Employee_MasterDAL objEmployee_MasterDAL;
		ResultPT objResultPT;
		#endregion
		
        #region Public Constructors
        public Employee_MasterBAL()
        {
            //
            // TODO: Add constructor logic here
            //
            //objEmployee_MasterDAL=new Employee_MasterDAL();
        }
        #endregion       

        #region Public Methods
        public async Task<ResultPT> InsertAsync(Employee_MasterDTO objEmployee_MasterDTO)
        { 
			try
            {
				objResultPT = new ResultPT();
				using (Employee_MasterDAL objEmployee_MasterDAL = new Employee_MasterDAL())
                {
                    objResultPT.ReturnObject = await objEmployee_MasterDAL.InsertAsync(objEmployee_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Employee_Master");
        ...
        public async Task<ResultPT> UpdateAsync(Employee_MasterDTO objEmployee_MasterDTO)
        {
			try
            {
				objResultPT = new ResultPT();
				using (Employee_MasterDAL objEmployee_MasterDAL = new Employee_MasterDAL())
                {
                    objResultPT.ReturnObject = await objEmployee_MasterDAL.UpdateAsync(objEmployee_MasterDTO);
                    if (Convert.ToInt32(objResultPT.ReturnObject) == 2)
                    {
                        objResultPT.ResultMsg = string.Format(Messages.ObjectExist, "Employee_Master");
                        objResultPT.TransactionStatus = Enums.ResultStatus.Warning;
        ...
        public async Task<ResultPT> DeleteAsync(long ID)
        {
			try
            {
				...
				using (Employee_MasterDAL objEmployee_MasterDAL = new Employee_MasterDAL())
                {
                    objResultPT.ReturnObject = await objEmployee_MasterDAL.DeleteAsync(ID);
...
////			using (Employee_MasterDAL objEmployee_MasterDAL = new Employee_MasterDAL())
////            {
////                 objResultPT.ReturnObject = objEmployee_MasterDAL.DeleteAll();
////                objResultPT.TransactionStatus = Enums.ResultStatus.Success;
...
////    }
        ...
        public async Task<ResultPT> GetByIDAsync(long ID)
        {
			try
            {
				...
				using (Employee_MasterDAL objEmployee_MasterDAL = new Employee_MasterDAL())
                {                    
					DataSet dset = await objEmployee_MasterDAL.GetByIDAsync(ID);
					if (dset.Tables.Count > 0 && dset.Tables[0].Rows.Count>0)
					{
                        DataTable dt = dset.Tables[0];
						objResultPT.ReturnObject = CommonFunctions.FillProperties<Employee_MasterDTO>(dt.Rows[0]);
						objResultPT.ResultMsg = Messages.Success;
                        objResultPT.TransactionStatus = Enums.ResultStatus.Success;
					}
					else 
        ...
        public async Task<ResultPT> GetAllAsync( )
        {
			try
            {
				...
				using (Employee_MasterDAL objEmployee_MasterDAL = new Employee_MasterDAL())
                {
                    objResultPT.ReturnObject = await objEmployee_MasterDAL.GetAllAsync();                   
                }
				return objResultPT;
			}
            catch (Exception Ex)
  ...
  //          using (Employee_MasterDAL objEmployee_MasterDAL = new Employee_MasterDAL())
  //          {
  //              return await objEmployee_MasterDAL.FilterAsync(param);
  //          }
  //      }
