...
/// <summary>
/// Summary description for User_MasterDAL
/// </summary>
namespace WebApiDAL 
{
    
    public class User_MasterDAL  : BaseDAL<User_MasterDTO>,IDisposable
    {

        public async Task<DataSet> Login(string UserName, string password)
        {
            SqlParameter[] arParms = new SqlParameter[2];
            arParms[0] = new SqlParameter("@UserName", SqlDbType.NVarChar)
            {
                Value = UserName
            };
            arParms[1] = new SqlParameter("@Password", SqlDbType.NVarChar)
            {
                Value = password
            };
            return await SqlHelper.ExecuteDatasetAsync(DBConStr, typeof(User_MasterDTO).Name.Replace("DTO", "") + SPConstant.User_LogIn, arParms);
        }
		...
		public const string SP_User_MasterFilter = "User_MasterFilter";    
...
//            DataSet dset = await SqlHelper.ExecuteDatasetAsync(DBConStr, SPConstant.SP_User_MasterFilter, Sqlparam);
